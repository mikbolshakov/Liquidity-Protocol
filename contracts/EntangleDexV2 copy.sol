...
contract EntangleDexV2 is Initializable, AccessControlUpgradeable {
    ...
    EntangleSynthFactory synthFactory;
    address dexWrapper;
    ...
    function buy(uint128 sid, address token, uint256 amount) public isAllowedToken(token) {
        ...
        EntangleSynth synth = synthFactory.synths(sid);
        uint256 synthAmount = synth.convertOpAmountToSynthAmount(opAmount);

        // 5. Проверяет принадлежит ли sid текущей сети.
        if (__chainId() == SidLibrary.peekChainId(sid)) {
    ...
    function sell(uint128 sid, uint256 synthAmount) public {
        EntangleSynth synth = synthFactory.synths(sid);
        uint256 opAmount = synth.convertSynthAmountToOpAmount(synthAmount);
