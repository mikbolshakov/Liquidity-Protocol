...
abstract contract BaseSynthChef is PausableAccessControl, Lender {
    ...
    function withdraw(
        ...
        for (uint i = 0; i < tokens.length; i++) {
            tokenAmount += _convertTokens(tokens[i].token, _toToken, tokens[i].amount);
        }
    ...
    function compound(uint256 _pid) public onlyRole(ADMIN_ROLE) whenNotPaused {
        ...
        for (uint i = 0; i < rewardTokens.length; i++) {
            ...
            if (balance > 0) {
                ...
                this.deposit(_pid, address(rewardTokens[i]), balance - feeAmount, 0);
    ...
    function convertTokenToStablecoin(
    ...
    function getBalanceOnFarm(uint256 _pid) public view returns (uint256 totalAmount) {
        TokenAmount[] memory tokens = _getTokensInLP(_pid);
        for (uint i = 0; i < tokens.length; i++) {
            if (tokens[i].amount > 0) {
                totalAmount += convertTokenToStablecoin(tokens[i].token, tokens[i].amount);
            }
        }
    }

    function _convertStablecoinAmountToLPAmount(
        uint256 _pid,
        uint256 _amount
    ...
    function _removeLiquidity(
        uint256 _pid,
        uint256 _amount
    ) internal virtual returns (TokenAmount[] memory tokenAmounts);
    ...
    function _getTokensInLP(
        uint256 _pid
    ) internal view virtual returns (TokenAmount[] memory tokens);

    function getLPAmountOnFarm(uint256 _pid) public view virtual returns (uint256 amount);
    ...
    function setRewardTokens(address[] memory _rewardTokens) public onlyRole(ADMIN_ROLE) {
        rewardTokens = _rewardTokens;
    }

    function setDEXWrapper(IEntangleDEXWrapper _DEXWrapper) public onlyRole(ADMIN_ROLE) {
