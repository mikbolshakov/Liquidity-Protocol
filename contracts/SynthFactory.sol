    ...
    OwnableUpgradeable
{
    ...
    function setPrice(
        ...
        uint256 price,
        uint256 totalSupply
    ) public validSynth(sid) onlyRole(SPOTTER) {
    ...
    function mint(
        ...
        address to,
        uint256 opId
    ) public validSynth(sid) onlyRole(MINTER) {
    ...
    function convertOpToSynth(
        uint128 sid,
        uint256 opAmount
    ) public view validSynth(sid) returns (uint256) {
        uint256 price = synths[sid].price;
        return
            ((opAmount * (10 ** synthDecimals)) / price) *
            (10 ** (synthDecimals - opTokenDecimals));
    }

    function convertSynthToOp(
        uint128 sid,
        uint256 synthAmount
    ) public view validSynth(sid) returns (uint256) {
        uint256 price = synths[sid].price;
        return
            ((synthAmount * price) / (10 ** synthDecimals)) /
            (10 ** (synthDecimals - opTokenDecimals));
    }
