...
contract EntangleDEX is PausableAccessControl, Lender {
    ...
    function add(EntangleSynth _synth) public onlyRole(ADMIN) whenNotPaused {
        require(!synths[_synth].isActive, "Already added");
        synths[_synth] = SynthData({isActive: true});
    }
    ...
    function buy(
        ...
        uint256 _amount
    ) public exist(_synth) onlyRole(BUYER) whenNotPaused returns (uint256 synthAmount) {
    ...
    function sell(
        EntangleSynth _synth,
        uint256 _amount
    ) public exist(_synth) onlyRole(BUYER) whenNotPaused returns (uint256 opTokenAmount) {
        IERC20 opToken = _synth.opToken();
     ...
     * - the caller must have admin role.
     */
    function switchSynthState(
        EntangleSynth _synth
    ) public exist(_synth) whenNotPaused onlyRole(ADMIN) {
